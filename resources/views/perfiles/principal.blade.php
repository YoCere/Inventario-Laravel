...
<table class="min-w-full border-collapse block md:table">
    ...
    <tbody class="block md:table-row-group">
        ...
        <tr class="bg-gray-300 border border-grey-500 md:border-none block md:table-row">
            <td class="p-2 md:border md:border-grey-500 text-left block md:table-cell"><span class="inline-block w-1/3 md:hidden font-bold">Nombre</span><a href="{{route('perfiles.mostrar',$perfil->id)}}">{{$perfil->nombre}}</a></td>
            <td class="p-2 md:border md:border-grey-500 text-left block md:table-cell"><span class="inline-block w-1/3 md:hidden font-bold">Precio</span>{{$perfil->precio}}</td>
            <td class="p-2 md:border md:border-grey-500 text-left block md:table-cell"><span class="inline-block w-1/3 md:hidden font-bold">Operaciones</span>      
                ...
                <form action="{{route('perfilles.borrar',$perfil)}}"  method="POST">
                    @method('delete')
                    @csrf
